...
# Build-time arg: GitHub owner/repo slug (override with --build-arg REPO_SLUG=owner/repo)
...
# ca-certificates: TLS roots for HTTPS fetch; deno: JS runtime for yt-dlp's EJS challenge solver
# (signature/n-parameter decryption — without it YouTube returns image-only formats).
# Then install latest stable yt-dlp from PyPI (--break-system-packages bypasses PEP 668 on Alpine).
RUN apk add --no-cache python3 py3-pip ffmpeg ca-certificates deno \
...
# Create /data owned by the unprivileged node user. When mounting -v <host>:/data,
# the host directory must be writable by uid 1000 or the SQLite open will fail.
...
# Drop root before exec'ing the app. yt-dlp + ffmpeg run fine unprivileged.
USER node
